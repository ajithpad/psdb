...
class Simulation(object):
    ...
    def __init__(self,prefix,new_pars=[],pars_file=[]):
	    ...
	    import params_hysterisis_stn
	    reload(params_hysterisis_stn)
	    pars = params_hysterisis_stn.Parameters(new_pars)		
	...
	for ii,nr in enumerate(pars.N_exc):
	  self.pops_exc[ii] = nest.Create(pars.model_type_exc, abs(nr))
	...
	for ii,nr in enumerate(pars.N_inh):
	  self.pops_inh[ii] = nest.Create(pars.model_type_inh, abs(nr))
	...
	if self.pars.change_type == 1:
	  for ii in self.time_lis:
	    if ii > self.pars.chg_time:
	      print 'changing type'
	      nest.SetStatus(self.pops[0][:self.pars.num_chg], params = {'spb':self.pars.chg_spk_bs})
